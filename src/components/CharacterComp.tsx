...
export default function CharacterComp() {
  ...
  const [schedule, setSchedule] = useState<ItemCover[]>([
    {
      season: "",
      title: "",
      id: 0,
          ...
          <div key={index}>
            ...
            {schedule[index].schedule.map((scheduleItem, scheduleIndex) => (
              <div key={scheduleIndex}>
                ...
                <input
                  type="text"
                  className={`title-${scheduleIndex}`}
                  onChange={(e) => handleTitleChange(e, index, scheduleIndex)}
                  value={scheduleItem.time}
                />
                내용
                <input
                  type="text"
                  className={`content-${scheduleIndex}`}
                  onChange={(e) => handleContChange(e, index, scheduleIndex)}
                  value={scheduleItem.content}
                />
                {scheduleIndex === 0 &&
                  schedule[index].schedule.length < 10 && (
                    <button onClick={() => addInput(index)}> + </button>
                  )}
                {scheduleIndex > 0 &&
                schedule[index].schedule[scheduleIndex - 1] ? (
                  <button onClick={() => deleteInput(index, scheduleItem.id)}>
                    -
                  </button>
                ) : (
                  ""
  ...
  function addInput(index: number) {
    const input: InputItem = {
      id: schedule[index].schedule.length,
      time: "",
      content: "",
    };
    const copydata = [...schedule];
    copydata[index].schedule.push(input);
    setSchedule(copydata);
  }
  function deleteInput(index: number, scheduleId: number) {
    const copydata = [...schedule];
    const copydata2 = copydata[index].schedule.filter(
      (item) => item.id !== scheduleId
    );
    copydata[index].schedule = copydata2;
    setSchedule(copydata);
  }
  function addCard() {
    const input = {
  ...
  function handleTitleChange(
    ...
    //setSchedule로는 별개의 속성을 바꿀 수 없으니 카피를 하나 만들어 수정하고 수정한 내용을 setState로 덮어쓴다.
    const copydata = [...schedule];
    copydata[index].schedule[scheduleIndex].time = e.target.value;
    setSchedule(copydata);
    console.log(copydata);
  }
  function handleContChange(
    e: ChangeEvent<HTMLInputElement>,
    index: number,
    scheduleIndex: number
  ) {
    const copydata = [...schedule];
    copydata[index].schedule[scheduleIndex].content = e.target.value;
    setSchedule(copydata);
    console.log(copydata);
  }
